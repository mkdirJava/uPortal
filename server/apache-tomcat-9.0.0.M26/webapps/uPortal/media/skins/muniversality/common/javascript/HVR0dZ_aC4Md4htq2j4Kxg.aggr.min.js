...
(function(a){a.fn.jqmAccordion=function(b){var d={showTabByDefault:true,tabIndexShown:0,tab:"li[data-role=list-divider]",slideUpSpeed:150,slideDownSpeed:300,closeTabs:false};var c=a.extend(d,b);if(c.showTabByDefault){setTimeout(function(){a(c.tab).eq(c.tabIndexShown).nextUntil(c.tab).show();},10);}this.delegate(c.tab,"click",function(){var g=a(this),h=g.next().is(":visible"),e=g.parent().children().not(c.tab),f=g.nextUntil(c.tab);if(h&&c.closeTabs){f.slideUp(c.slideUpSpeed);}else{if(h){return;}else{e.slideUp(c.slideUpSpeed);f.slideDown(c.slideDownSpeed);}}});return this;};})(jQuery);